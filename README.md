...
Everyone plays in the same world and spawns at the same place.

- **LAN game (Windows):** start `ClaudeCraft.exe`, open a single-player world, press **Esc** and click **Open to LAN**. The chat shows the address to share (for example `http://192.168.1.20:25565/`). Friends on the same network open that address in their browser, or use **Multiplayer → Join Server** with it. When you quit, the game ends for everyone.
- **Always-on server:** with [Node.js](https://nodejs.org) 22+, run `npm run server` (or `node tools/server.mjs --port 25565 --seed mySeed --mode survival`) from the source code. The world is saved to `worlds/server.json`, and players join at `http://<server address>:25565/`. `node tools/server.mjs --lan` runs a relay for "Open to LAN" instead, if you play from the `.html` file.
